...
NEAR_KEEP_LY = 50.0
DIM_CAP_ABSMAG = 8.0
BEACON_MAG = 2.7        # famous-landmark cut: naked-eye bright beyond the bubble
...
import re
LC_RE = re.compile(r'(Ia|Ib|IV|III|II|I|V)')
LC_CODE = {'Ia': 1, 'Ib': 1, 'I': 1, 'II': 2, 'III': 3, 'IV': 4, 'V': 5}


def lc_of(spect):
    """Luminosity class from the spectral string: 1 supergiant .. 5 main seq."""
    m = LC_RE.search(spect[1:])
    return LC_CODE[m.group(1)] if m else 0
...
def main():
    ...
    with open(path) as f:
        for r in csv.DictReader(f):
            ...
            if dpc <= 0 or dpc >= 90000:
                continue
            dly = dpc * LY_PER_PC
            is_beacon = (dly > MAX_LY and r['proper'] and r['mag'] and
                         float(r['mag']) <= BEACON_MAG)
            if dly > MAX_LY and not is_beacon:
                continue
            ...
            for ch in spect[1:3]:
                if ch.isdigit():
                    sub = int(ch)
                    break
            (beacons if is_beacon else stars).append({
    ...
    def row(s):
        ...
        return '[%s,%.2f,%.2f,%.2f,%s,%.1f,%d,%s,%d,%s,%.1f,%d,%d],\n' % (
            json.dumps(s['name']),
            s['x'], s['y'], s['z'],
            json.dumps(cls),
            s['absmag'], s['temp'],
            json.dumps(s['con']),
            1 if s['named'] else 0,
            '%.3g' % s['lum'], s['mag'], s['var'], s['lc'])

    beacons.sort(key=lambda s: s['dly'])
    ...
    with open(out, 'w') as f:
        ...
        f.write('// [name, x, y, z (ly, galactic), class, absmag, temp K, '
                ...
                '//  luminosity class (1 supergiant .. 5 main sequence)]\n')
        f.write('export const STARS = [\n')
        for s in cat:
            f.write(row(s))
        f.write('];\n')
        f.write('// famous landmarks beyond the bubble: proper-named, '
                'mag <= %.1f\n' % BEACON_MAG)
        f.write('export const BEACONS = [\n')
        for s in beacons:
            f.write(row(s))
        f.write('];\n')
    print(f"wrote {out}: {len(cat)} systems + {len(beacons)} beacons, "
